function fig = plot_difference_topographies(topo_data, stats_results, ...
    window_label, condition_pairs, channels, layout, pub)
%PLOT_DIFFERENCE_TOPOGRAPHIES Plot difference maps with significant clusters.
%   FIG = PLOT_DIFFERENCE_TOPOGRAPHIES(...) creates a 1 x N figure with
%   condition-difference topographies using a symmetric diverging colormap.
...
%     fig - figure handle
    ...
    fprintf('  Creating difference maps with clusters...\n');
    ...
    % Global symmetric color limits across all differences
    all_diffs = [];
    for p = 1:n_pairs
        cond1_data = topo_data.(window_label).data.(condition_pairs{p,1}).data;
        cond2_data = topo_data.(window_label).data.(condition_pairs{p,2}).data;
        all_diffs = [all_diffs; cond1_data(:) - cond2_data(:)]; %#ok<AGROW>
    end
    max_abs = max(abs(all_diffs));
    zlim_diff = [-max_abs, max_abs];

    cb_arr = cell(n_pairs, 1);
    for p = 1:n_pairs
        subplot(1, n_pairs, p);
        cb_arr{p} = plot_single_difference(topo_data, stats_results, ...
            condition_pairs{p,1}, condition_pairs{p,2}, condition_pairs{p,3}, ...
            window_label, channels, layout, pub, zlim_diff);
    end
...
function cb = plot_single_difference(topo_data, stats_results, cond1, cond2, ...
    title_str, window_label, channels, layout, pub, zlim_range)

    data_diff      = topo_data.(window_label).data.(cond1).data - ...
                     topo_data.(window_label).data.(cond2).data;
    ...
    ft_data = struct('label', {channels(:)}, 'avg', data_diff(:), ...
    ...
    cb = colorbar;
    ylabel(cb, '\Delta Power (dB)', 'FontName', pub.font_name, 'FontSize', pub.font_size_label);
...
function overlay_significant_clusters(stats_results, window_label, comp_name, channels, layout)
    % Mark significant cluster electrodes with black stars
